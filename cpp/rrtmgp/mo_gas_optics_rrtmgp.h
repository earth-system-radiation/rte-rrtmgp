...
class GasOpticsRRTMGPK : public OpticalPropsK<RealT, LayoutT, DeviceT> {
 public:
  ...
  void init_abs_coeffs(GasConcsK<RealT, LayoutT, DeviceT> const &available_gases,
    ...
    Kokkos::parallel_for( mdrp_t::template get<2>({this->flavor.extent(0), this->flavor.extent(1)}) , KOKKOS_LAMBDA (int i, int j) {
  ...
  void compute_gas_taus(bool top_at_1, int ncol, int nlay, int ngpt, int nband,
    ...
    //   do ilay = 1, nlay
    //     do icol = 1, ncol
    Kokkos::parallel_for( mdrp_t::template get<3>({ngas,nlay,ncol}) , KOKKOS_LAMBDA (int igas, int ilay, int icol) {
      col_gas(icol,ilay,igas) = vmr(icol,ilay,igas) * col_dry_wk(icol,ilay);
    });
    // ---- calculate gas optical depths ----
  ...
  void source(bool top_at_1, int ncol, int nlay, int nbnd, int ngpt,
    ...
    if (tlev.is_allocated()) {
      ...
      // do ilay = 1, nlay+1
      //   do icol = 1, ncol
      Kokkos::parallel_for( mdrp_t::template get<2>({nlay+1,ncol}) , KOKKOS_LAMBDA (int ilay, int icol) {
        if (ilay == 0) {
  ...
  void get_col_dry(VmrT const &vmr_h2o, PlevT const &plev, G0T const& g0, ColDryT const& col_dry, LatT const &latitude=LatT()) {
    ...
    Kokkos::parallel_for( mdrp_t::template get<2>({nlev-1,ncol}) , KOKKOS_LAMBDA (int ilev , int icol) {
